...
#### OPTIONS
    ...
    my $CFG_OUT     = qq[$BASE_DIR/runtime];
        ...
        for my $cfg_item_type ( keys %{ $CFG_ITEM_TYPES } )
        {
            ...
            for my $cfg_item ( @{ $configs->{$cfg_item_type} } )
            {
                ...
                for my $site ( @{ $cfg_item->{sites} } )
                {
                    ...
                    # элементы с полями name и use, содержащими "site1", не должны попадать в конфиги с именами файлов, содержащими "site2", и наоборот
                    if(
                        (
                            defined( $cfg_item->{name} ) and $cfg_item->{name} =~ m{^site(?<site>[0-9]+)}
                        )
                        or
                        (
                            defined( $cfg_item->{use} ) and $cfg_item->{use} =~ m{^site(?<site>[0-9]+)}
                        )
                    )
                    {
                        next if $site != $+{site};
